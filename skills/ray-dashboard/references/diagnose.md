...
1. `status` 探测：`running: false` → 根因是服务没在跑，恢复命令是 `start`（诊断请求只给命令不代跑）；`running: true` 则先核对 healthz 返回的 vault 名——**必须与用户的 vault 一致才是"用户的工作台"**，同机可能跑着别的库的实例，指错等于把用户送进别人的库。vault 名一致才继续排查浏览器侧问题（换无痕窗口排除扩展）。
...
- "这张审核卡格式不完整"：卡片缺人工审核选项，去 Obsidian 里检查该文件，通常是手工编辑破坏了格式。
- "审核功能未启用"或卡片只读、没有操作按钮：`50-系统/40-自动化/` 下缺 `review_protocol.json`（healthz 返回 `review_enabled: false` 可确认）。重跑 `install` 会补默认协议，补上后重启工作台。看板流转按钮消失同理，对应 `board_protocol.json`。
